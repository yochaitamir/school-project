...
class AdminForm
{
    ...
    public function editForm()
    {
        if (isset($_GET['edituser'])&&$_GET['edituser']=='edit') {
            ...
            foreach ($user as $use) {
                $use=$use;
                $_SESSION['userid']=$use['id'];
                $_SESSION['rolename']=$use['rolename'];
        ...
        echo '<form  class="topformtest" action="" method="POST" enctype="multipart/form-data">
                   <br>';
        echo'<br><div class=labelwidth><label>User name</label></div>';
        ...
        if ((isset($_GET['edituser'])&&$_GET['edituser']=='adduser')||$_SESSION['role']=='owner') {
            ...
            echo '<label>manager </label><input type="radio" name="role" value="manager"';
            if (isset($_GET['edituser'])&&$_GET['edituser']=='edit'&&$use['rolename']=='manager') {
                echo 'checked="checked"';
            }
            '><br>';
            echo '<label>salesman</label><input type="radio" name="role" value="sales"';
            if ($_GET['edituser']!='edit') {
                echo 'required="required"';
            }
            if (isset($_GET['edituser'])&&$_GET['edituser']=='edit'&&$use['rolename']=='sales') {
                echo 'checked="checked"';
            }
            ' ><br>';
            echo '</div><br>';
        ...
        echo '<div id="wrapper">';
        echo '<input  id="fileUpload"  type="file" name="imgfile"';
        if ($_GET['edituser']!='edit') {
            echo 'required';
        }
        echo '><br>';
    ...
    public function affirmingDeleteUser()
    {
        echo 'are you sure you want to delete  user '.$_SESSION['userid'];
        echo '<form action="" method="POST">';
        echo  '<input type="submit" name="affirmingdeleteuser" value="delete">';
        echo '<input type=submit name="canceldeleteuser" value="cancel">';
        echo '</form>';
    }
}
